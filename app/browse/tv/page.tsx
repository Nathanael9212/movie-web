...
export default async function TVShowsPage({
  ...
  let popularQuery = "/tv/popular?page=1";
  let topRatedQuery = "/tv/top_rated?page=1";
  ...
  if (selectedRating !== "all" || selectedGenre !== "all") {
    ...
    popularQuery = `/discover/tv?sort_by=popularity.desc${
      selectedRating !== "all" ? `&vote_average.gte=${minRating}&vote_count.gte=100` : ""
    }${genreParam}&page=1`;
    
    topRatedQuery = `/discover/tv?sort_by=vote_average.desc${
      selectedRating !== "all" ? `&vote_average.gte=${minRating}&vote_count.gte=100` : ""
    }${genreParam}&page=1`;
  }
        ...
        <div className="mb-12">
          ...
          <div className="mb-6">
            ...
            <div className="flex gap-2 flex-wrap">
              ...
              {TV_GENRES.map((genre) => (
                <Link
                  key={genre.id}
                  href={`/browse/tv?genre=${genre.id}${selectedRating !== "all" ? `&rating=${selectedRating}` : ""}`}
                  className={`px-4 py-2 rounded-full text-sm font-semibold transition-all ${
          ...
          <div>
            ...
            <div className="flex gap-3 flex-wrap">
              <Link
                href={`/browse/tv${selectedGenre !== "all" ? `?genre=${selectedGenre}` : ""}`}
              ...
              <Link
                href={`/browse/tv?rating=8${selectedGenre !== "all" ? `&genre=${selectedGenre}` : ""}`}
              ...
              <Link
                href={`/browse/tv?rating=7${selectedGenre !== "all" ? `&genre=${selectedGenre}` : ""}`}
                className={`px-5 py-2.5 rounded-full font-semibold transition-all flex items-center gap-2 ${
                  selectedRating === "7"
              ...
              <Link
                href={`/browse/tv?rating=6${selectedGenre !== "all" ? `&genre=${selectedGenre}` : ""}`}
                className={`px-5 py-2.5 rounded-full font-semibold transition-all flex items-center gap-2 ${
                  selectedRating === "6"
          ...
          {popular.results.length > 0 ? (
            ...
            <div className="text-center py-20 bg-gray-50 rounded-2xl">
              <p className="text-gray-600 text-lg">
                No TV shows found with selected filters
              </p>
            </div>
          )}
        </section>
